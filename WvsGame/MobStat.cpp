...
void MobStat::EncodeTemporary(OutPacket *oPacket, TS_Flag & dwFlag, int tCur) {
	...
	if (CHECK_MS_NORML(dwFlag, MobStatNumbers::MOB_STAT_Burned)) {
		...
		//		oPacket->Encode4(buff.getSkill()); // nSkillID 技能ID
		//		oPacket->Encode4(7100); // nDamage 每秒傷害?
		//		oPacket->Encode4(1000); // tInterval 延遲毫秒 : dotInterval * 1000
		...
		//		oPacket->Encode4(7100); // nDotTickIdx 每秒傷害?
